...
class purchase_order(osv.osv):  
    ...
    _track = {
        'state': {
            ...
            'ydit_pur_workflow.mt_rfq_rejected': lambda self, cr, uid, obj, ctx=None: obj['state'] == 'rejected',
        },
    }
...
#        lines = self._get_lines(cr,uid,ids,['confirmed'],context=context)
    ...
    def action_cancel_draft(self, cr, uid, ids, context=None):
        for po in self.browse(cr, uid, ids, context=context):
            if po.state not in('cancel','sent','confirmed','3rd'):
    ...
    def button_to_changing(self, cr, uid, ids, context=None):
        ...
        for purchase in self.browse(cr, uid, ids, context=context):
            '''
            08/24/2015, johnw, change the logic, 
            for the picking/inv not in draft state, user need manual cancel the related picking and invoices
            for the picking/inv with draft state, system cancel them direct, not to delete
            '''
            for pick in purchase.picking_ids:
...
def unlink_ydit_po_line(self, cr, uid, ids, context=None):
